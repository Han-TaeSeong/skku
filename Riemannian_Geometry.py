...
#     for i in range(0, 288):
#         scm = x_train[i]
#         scm = (np.matmul(scm, scm.transpose())) / 649  #### time_series -1 로 나누어 주기(scaling)
#         if i == 0:
...
for z in range(1,10):
    z = str(z)
    train_file = np.load('.\\Preprocessed_Data\\4_100Hz\\A0'+z+'T.npz')
    test_file = np.load('.\\Preprocessed_Data\\4_100Hz\\A0'+z+'E.npz')
    x_train = train_file['x']
    y_train = train_file['y']
    x_test = test_file['x']
    y_test = test_file['y']
    ...
    for i in range(0, 288):
        scm = x_train[i]
        scm = (np.matmul(scm, scm.transpose())) / 650  #### time_series  로 나누어 주기(scaling)
        if i == 0:
            x_train_SCM = np.expand_dims(scm, axis=0)
        else:
            scm = np.expand_dims(scm, axis=0)
